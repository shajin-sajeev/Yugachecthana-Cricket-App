...
import { Camera, Mail, Phone, MapPin, Award, Edit2, Save, Share2, TrendingUp, Trophy, Calendar, User } from 'lucide-react';
        ...
        return (
          <div className="space-y-6 animate-in fade-in duration-300">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {[
                { label: 'Matches', value: user.matches, icon: Calendar, color: 'text-blue-500' },
                { label: 'Runs', value: user.runs, icon: TrendingUp, color: 'text-orange-500' },
                { label: 'Batting Avg', value: user.avg, icon: Award, color: 'text-purple-500' },
                { label: 'MVP Points', value: user.mvpPoints, icon: Trophy, color: 'text-gold' },
                ].map((stat, i) => (
             ...
             <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700">
                 ...
                 <div className="space-y-4">
                    <div className="flex justify-between py-2 border-b border-gray-100 dark:border-gray-700">
                        ...
                        <span className="font-bold text-gray-900 dark:text-white">{user.runs}</span>
                    </div>
                    <div className="flex justify-between py-2 border-b border-gray-100 dark:border-gray-700">
                        <span className="text-gray-500 dark:text-gray-400">Highest Score</span>
                        <span className="font-bold text-gray-900 dark:text-white">{user.highestScore}</span>
                    </div>
                    <div className="flex justify-between py-2 border-b border-gray-100 dark:border-gray-700">
                        <span className="text-gray-500 dark:text-gray-400">Average</span>
                        <span className="font-bold text-gray-900 dark:text-white">{user.avg}</span>
                    </div>
                    <div className="flex justify-between py-2 border-b border-gray-100 dark:border-gray-700">
                        <span className="text-gray-500 dark:text-gray-400">Strike Rate</span>
                        <span className="font-bold text-gray-900 dark:text-white">{user.strikeRate}</span>
                    </div>
                    <div className="flex justify-between py-2 border-b border-gray-100 dark:border-gray-700">
                        <span className="text-gray-500 dark:text-gray-400">100s / 50s</span>
                        <span className="font-bold text-gray-900 dark:text-white">{user.centuries} / {user.fifties}</span>
                    </div>
                 </div>
             </div>
        ...
        <div className="pt-16 px-6 sm:px-8 pb-8">
          <div className="grid md:grid-cols-2 gap-8">
            <div className="space-y-4">
              <div>
                {isEditing ? (
                  <input type="text" defaultValue={user.name} className="w-full mt-1 p-2 border rounded-lg dark:bg-gray-700 dark:border-gray-600 dark:text-white font-bold text-lg" />
                ) : (
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-gray-50 dark:bg-gray-700/50 p-3 rounded-xl border border-gray-100 dark:border-gray-700">
                  <div className="text-xs text-gray-500 dark:text-gray-400">Batting Style</div>
                  <div className="font-semibold text-gray-900 dark:text-white text-sm">{user.battingStyle}</div>
                </div>
                <div className="bg-gray-50 dark:bg-gray-700/50 p-3 rounded-xl border border-gray-100 dark:border-gray-700">
                  <div className="text-xs text-gray-500 dark:text-gray-400">Bowling Style</div>
                  <div className="font-semibold text-gray-900 dark:text-white text-sm">{user.bowlingStyle}</div>
                </div>
              </div>
            </div>
          </div>
